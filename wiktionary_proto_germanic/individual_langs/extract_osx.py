...
osx = gem_unimorph_df[gem_unimorph_df.lang == "osx"]
osx_incomplete = osx[osx["PRET.SG"].isnull()]
osx_incomplete.dropna(subset=["links"], inplace=True)
...
def find_conjugation(soup, conjugation_id="Conjugation", i=0):
    ...
    if 'lang="osx"' not in table.find("td").prettify() or table is None:
        return find_conjugation(soup, i=i + 1)
    else:
        return table
...
for link in tqdm(osx_incomplete["links"]):
    ...
    for i, row in enumerate(rows):
        ...
        elif current_row_val == "plural" and first_person_plural_past is None:
            first_person_plural_past = rows[i].find_all("td")[-1].text.strip()
        else:
            continue

    gem_unimorph_df.loc[gem_unimorph_df.links == link, "PRET.SG"] = pick_one_variant(
        third_person_singular_past
    )
    gem_unimorph_df.loc[gem_unimorph_df.links == link, "PRET.PL"] = pick_one_variant(
        first_person_plural_past
    )
    gem_unimorph_df.loc[gem_unimorph_df.links == link, "PTCP"] = pick_one_variant(
        past_participle
    )
