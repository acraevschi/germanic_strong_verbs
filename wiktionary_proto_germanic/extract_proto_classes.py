...
### Proto classes were not initally extracted from the Wiktionary page, so we need to do it now
...
for i in range(1, 8):
    ...
    pages = get_urls(page)
    lang_urls.extend(pages)
    class_n.extend([i] * len(pages))
    if i in (5, 6, 7):
        page = urlopen(
...
cognates = dict()
for url, class_n in tqdm(zip(lang_urls, class_n), desc="Extracting proto-classes"):
    page = urlopen(url)
    html = page.read()
    page.close()
    soup = BeautifulSoup(html, "lxml")
    ...
    cognates[proto_gem_form] = class_n

cognates = pd.DataFrame.from_dict(cognates, orient="index"y, colnames=["class_n"])
cognates.to_csv("proto_classes.csv", encoding="UTF-8")