...
output_dir <- paste("../analysis_results/simmap_verb_hierarchy_root", sep="")
...
file.dir <- paste("../analysis_results/tam_simmap_root/", 
                   "summ_simmap_mcc.rds", sep="")

tree <- readRDS(file.dir) # all trees already rescaled
...
proto_gem_patterns <- read.csv("../anc_rec/proto_gem_forms/proto_gem_forms.csv", stringsAsFactors=FALSE)
proto_gem_patterns$proto_gem_form <- proto_gem_patterns$form
proto_gem_patterns$lang <- "Proto-Germanic"

# append the proto-Germanic patterns to the patterns
patterns <- rbind(patterns, proto_gem_patterns, fill=TRUE)
...
model_path = "models_code/simmap_verb_hierarchy.stan"
...
fit <- rstan::stan(
  file = model_path,
  data = data.list,
  iter = 5000, # many more params, hence higher iter
  warmup = 3000,
  chains = 4,
  cores=24, 
...
saveRDS(fit, paste(output_dir, "/mcc_tree_root.rds", sep=""))
