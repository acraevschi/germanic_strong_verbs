functions {
  array[] matrix fill_matrix(vector beta, int J) {
    ...
    vector[2*(J-1)*(J-2)] rho = beta[1:(2*(J-1)*(J-2))];
    real death_rate = beta[(2*(J-1)*(J-2))+1];
    {
      int k = 1;
      for (n in 1:2){
        for (i in 1:(J-1)) {
  ...
  vector pruning_likelihood( vector beta , vector theta , data real[] xr , data int[] xi ) {
    ...
    rates[1:(2*(J-1)*(J-2))] += theta;
    // to try:
    // rates[1:(2*(J-1)*(J-2))] = exp(rates[1:(2*(J-1)*(J-2))]);
    rates = exp(rates);
    array[2] matrix[J,J] Q = fill_matrix(rates, J);
    matrix[N,J] lambda;
    for (j in 1:J) {
...
parameters {
  vector[2*(J-1)*(J-2)] mu;
  vector<lower=0>[2*(J-1)*(J-2)] sigma;
  // to try:
  // real<lower=0> log_death_rate;
  real log_death_rate;
  vector[2*(J-1)*(J-2)] eps[V];
}
transformed parameters {
  vector[V] log_lik;
  vector[2*(J-1)*(J-2)] theta[V];
