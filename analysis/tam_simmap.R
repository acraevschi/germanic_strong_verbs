...
file_name <- paste("../trees/gem_trees_final.trees", sep="")

trees <- read.nexus(file_name)

data <- read.csv("../tam_char.csv", encoding = "UTF-8", stringsAsFactors=FALSE)
...
set.seed(97)
trees_sample <- sample(trees, n.tree_samples)
rm(trees)
  
for (t in 1:n.tree_samples){
  simmap_lst <- list()
  tree <- trees_sample[[t]]
  ...
  saveRDS(
    singleton.simmap, 
    paste("../analysis_results/tam_simmap_summary/summ_simmap_t_", t, ".rds", sep="")
    )
}
